...
// added stream
#include "util_thread_padding.cu"
#include <stdio.h>
#include <stdlib.h>
...
__device__ void generatePredictedBlockPad(unsigned char *d_temp_frame, unsigned char *prevFrame,
                                       int best_x, int best_y, int x_idx, int y_idx)
{
    for (int y = y_idx; y < y_idx + BLOCK_SIZE; y++)
    {
        for (int x = x_idx; x < x_idx + BLOCK_SIZE; x++)
        {
            int srcX = x + best_x + PADDING;
            int srcY = y + best_y + PADDING;

            // if (srcX >= 0 && srcX < WIDTH && srcY >= 0 && srcY < HEIGHT) {}
            d_temp_frame[y * WIDTH + x] = prevFrame[srcY * PADWIDTH + srcX];
        }
    }
}
...
__global__ void firstPFrameReconstGPU(unsigned char *d_curr_frame, unsigned char *d_ref_pad_frame, unsigned char *d_ref_frame,
                                      unsigned char *d_temp_frame, int *d_motion_vectors)
{
    ...
    for (int y = 0; y < HEIGHT; y++)
    {
        ...
        {
            d_ref_pad_frame[(y + PADDING) * PADWIDTH + (x + PADDING)] = d_ref_frame[y * WIDTH + x];
        }
    }
    ...
    int x_idx_p = x_block * BLOCK_SIZE + PADDING;
    int y_idx_p = y_block * BLOCK_SIZE + PADDING;
    ...
    {
        int best_sad = calculatePadSAD(d_curr_frame, d_ref_pad_frame, x_idx, y_idx, x_idx + PADDING, y_idx + PADDING);
        ...
        for (int i = 0; i < search_num; i++)
        {
            ...
            int x_ref = x_idx_p + RANGE_8[i][1];
            int y_ref = y_idx_p + RANGE_8[i][0];

            int sad = calculatePadSAD(d_curr_frame, d_ref_pad_frame, x_idx, y_idx, x_ref, y_ref);
        ...
        generatePredictedBlockPad(d_temp_frame, d_ref_pad_frame, best_x, best_y, x_idx, y_idx);
...
__global__ void nextPFrameReconstGPU (unsigned char *d_curr_frame, unsigned char *d_ref_pad_frame, unsigned char *d_ref_frame,
                                      unsigned char *d_temp_frame, int *d_motion_vectors)
{

    // unsigned char *temp_curr_frame_data = (unsigned char *)malloc((size_t)(PADWIDTH * PADHEIGHT) * sizeof(unsigned char));
    // memset(temp_curr_frame_data, 0, PADWIDTH * PADHEIGHT);

    for (int y = 0; y < HEIGHT; y++)
    {
        for (int x = 0; x < WIDTH; x++)
        {
            d_ref_pad_frame[(y + PADDING) * PADWIDTH + (x + PADDING)] = d_temp_frame[y * WIDTH + x];
        }
    }
...
void block_match_full_frame_stream(Frame* allYFrames) {
    //int *h_motion_vectors;

    unsigned char *d_curr_frame[STREAM_NUM];
    unsigned char *d_ref_pad_frame[STREAM_NUM];
    ...
    // size_t frame_size = padedWidth * padedHeight * sizeof(unsigned char);
    ...
    for (int i = 0; i < STREAM_NUM; i++) {
        ...
        cudaMalloc(&d_ref_pad_frame[i], PADWIDTH * PADHEIGHT * sizeof(unsigned char));
        cudaMemset(d_ref_pad_frame[i], 0, PADWIDTH * PADHEIGHT * sizeof(unsigned char));
    }
    ...
    for (int i = 0; i < STREAM_NUM; i++) {
        ...
        cudaMemcpyAsync(d_curr_frame[i], allYFrames[0 + GOP *i].y, frame_size, cudaMemcpyHostToDevice, stream[i]);
        ...
        firstPFrameReconstGPU<<<block, grid, 0, stream[i]>>>(d_curr_frame[i], d_ref_pad_frame[i], d_ref_frame[i], d_temp_frame[i], d_motion_vectors[i]);

        //cudaMemcpyAsync(allRconstFrames + gop_size * i, d_temp_frame[i], frame_size, cudaMemcpyDeviceToHost, stream[i]);
    ...
    for(int j = 1; j < GOP; j++) {
        for (int i = 0; i < STREAM_NUM; i++) {
            ...
            nextPFrameReconstGPU<<<block, grid, 0, stream[i]>>>(d_curr_frame[i], d_ref_pad_frame[i], d_ref_frame[i], d_temp_frame[i], d_motion_vectors[i]);
...
void block_match_full_frame_stream_save_reconst(Frame* allYFrames, unsigned char* allRconstFrames) {
    ...
    size_t pixel_num = WIDTH * HEIGHT;
    // size_t pixel_num = padedWidth * padedHeight;
    size_t gop_size = GOP * pixel_num;
    size_t frame_size = WIDTH * HEIGHT * sizeof(unsigned char);
    // size_t frame_size = padedWidth * padedHeight * sizeof(unsigned char);
    size_t sad_size = x_block_num * y_block_num * sizeof(int);
