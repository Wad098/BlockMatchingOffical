...
#define CUDA_BLOCK_LENGTH (int)(2 * SEARCH_RANGE + 1)
#define CUDA_BLOCK_SIZE (int)(CUDA_BLOCK_LENGTH * CUDA_BLOCK_LENGTH)
#define SHARED_BLOCK_SIZE (int)(2 * SEARCH_RANGE + BLOCK_SIZE)
#define WIDTH 352 // 1920
#define HEIGHT 288 // 1080


#define QP 6
#define FRAME_NUM 3000 // 3000
#define GOP 30
...
__constant__ int DIRECTION1_4[4][2] = {{16, 16}, {0, 16}, {16, 0}, {0, 0}};

__constant__ int DIRECTION2_4[4][2] = {{8, 8}, {-8, 8}, {8, -8}, {-8, -8}};
...
{
    int padding = PADDING;
    int new_width = width + padding *2 ;
    int new_height = height + padding *2;

    Frame *frame = (Frame *)malloc(sizeof(Frame));
    frame->y = (unsigned char *)malloc((size_t)(new_width * new_height) * sizeof(unsigned char));

    // Initialize all pixels to 255
    memset(frame->y, 255, new_width * new_height);

    // Read the file into the center of the frame
    for (int i = 0; i < height; i++)
    {
        fread(frame->y + (i + padding) * new_width + padding, 1, width, file);
    }
...
void saveAllFrameToText(FILE *file, unsigned char *frame) {
    ...
    int frame_size = PADWIDTH * PADHEIGHT;
    ...
    for (int frameNumber = 0; frameNumber < FRAME_NUM; frameNumber++) {
        ...
        for (int y = 0; y < PADHEIGHT; y++) {
            for (int x = 0; x < PADWIDTH; x++) {
                fprintf(file, "%d ", frame[y * PADWIDTH + x + frame_size * frameNumber]); 
            }
            fprintf(file, "\n"); 
        }
